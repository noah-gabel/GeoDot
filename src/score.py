...
def haversine_distance(guess_coordinates : Coordinates , city_coordinates : Coordinates) -> float:
    ...
    delta_phi = math.radians(city_coordinates.lat - guess_coordinates.lat)      # delta länge
    delta_lambda = math.radians(city_coordinates.lon - guess_coordinates.lon)   # delta breite

    # Haversine formula
    # value between 0 and 1
    # describes the distance measured through the earth
    # 0 = both points are at the same position
    # 1 = both points are exactly at teh opposite sides of the earth
    length = (
        math.sin(delta_phi / 2) ** 2
        + math.cos(phi1) * math.cos(phi2) * math.sin(delta_lambda / 2) ** 2
    )

    # angle form the earths center
