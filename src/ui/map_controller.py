...
from config import (
    ...
    CITY_MARKER_COLOR_INSIDE,
    CITY_MARKER_COLOR_OUTSIDE,
    FONT,
    GUESS_MARKER_CIRCLE_COLOR,
    GUESS_MARKER_COLOR,
    MARKER_TEXT_COLOR,
    RESULT_PATH_COLOR,
    RESULT_PATH_WIDTH,
...
from models import BoundingBox, Coordinates, Difficulty, Guess
...
class MapController(TkinterMapView):
    ...
    def __init__(
        self,
        master,
        enable_guess_button=None,
        disable_guess_button=None,
        corner_radius=0,
        allow_guessing=True,
    ):
        ...
        self.set_tile_server(
            f"https://a.basemaps.cartocdn.com/rastertiles/voyager_nolabels/{{z}}/{{x}}/{{y}}.png?key={API_KEY}"
        )
    ...
    def _on_release(self, event: Event):
        ...
        coords_x, coords_y = self.convert_canvas_coords_to_decimal_coords(
            canvas_x=self.pressed_pos.x, canvas_y=self.pressed_pos.y
        )
    ...
    def show_guess_result(self, guess: Guess):
        ...
        bounding_box = BoundingBox.from_points(
            points=[guess.coords, guess.city.coords]
        ).padded()
    ...
    def _fit_to_bounding_box(self, bounding_box: BoundingBox):
        ...
        self.fit_bounding_box(
            ...
            position_bottom_right=(
                bounding_box.se_corner.lat,
                bounding_box.se_corner.lon,
            ),
        )
